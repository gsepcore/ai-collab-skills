...
Agents can also hold natural conversations: task-specific threads live at `.ai-collab/thread-{task_id}.md`, and broader design/review discussions live at `.ai-collab/discussions/*.md`. `@slug` mentions in either place are delivered immediately by `ai-collab-converse.py` itself when you post them — attended, on the spot, no background process required — and are also picked up by the background daemon as a redundant catch-all if the user has opted into it.
...
9. Start `/collab monitor` automatically for this project in the current Claude Code session. Do not ask the user to run it manually. If a monitor for this project is already active, keep it and report "monitor already active." If the current runtime cannot launch a persistent Monitor/Task, say that clearly and rely on the prompt hooks (`UserPromptSubmit`/`Stop`) as the fallback — plus the background daemon if the user has explicitly opted into it with `AI_COLLAB_ENABLE_DAEMON=1`, since it is off by default.
...
**Behavior:** If the background daemon is enabled (off by default), it also runs this periodically when `AI_COLLAB_AUTO_UPDATE` is not `0`. Otherwise this only ever runs when invoked directly. Use `/collab update` for immediate recovery after a release or when a user suspects stale project rules.
...
5. Once the authorization line lands, close the thread yourself — collaboration is attended by default, so nothing scans for it in the background: run `python3 ~/.claude/ai-collab-wakeup.py "$ROOT" "$THREAD_PATH"` once against the debate thread; its closing-marker detection sets `status: closed` on that single pass. Read that final `type: decision` message in full.
...
**How other AIs respond:** Every AI checks `inbox-{its-name}.md` and `inbox-all.md` at the start of every response. If `status: unread`, the agent first sets `status: claimed` + `claimed_by` + `claimed_at`, then executes the task, then marks `status: done` and sets `done_at`. That attended check is what actually delivers tasks; if the background daemon is also enabled, it redundantly scans inboxes and task threads too, creating wake events for direct tasks and `@slug` mentions so configured adapters can activate each agent's monitor path. See `claude-task-lifecycle-spec.md` for the full state machine, conflict resolution, and director semantics.
...
- Do not overwrite a `running` task that is making progress. Treat a claim as stale only after 30 minutes with no update, by your own check of `claimed_at`/`updated`, not because something else declared it stale.
